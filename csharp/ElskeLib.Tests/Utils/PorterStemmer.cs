	...
	*/
    ...
    public class PorterStemmer
	{
		...
		*/
        ...
        public string Stem(string text, bool convertToLowerCase = false)
        {
            ...
            for (int l = 0; l < text.Length; l++)
            {
                ...
                var isLetter = char.IsLetterOrDigit(ch) 
                               || ch == '-' && wasLetter && l+1 < text.Length && char.IsLetterOrDigit(text[l+1]);
                
				if (isLetter)
                {
					add(convertToLowerCase ? char.ToLowerInvariant(ch) : ch);
                }

                wasLetter = isLetter;

				if (isLetter && l != text.Length - 1) continue;
